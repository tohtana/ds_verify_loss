...
| `--chunked_causal_lm_loss_empty_cache` | `false` | Empty the CUDA cache before chunked causal-LM loss upcasts |
| `--chunked_causal_lm_loss_device` | `cuda` | Device for chunked causal-LM cross entropy (`cuda` or `cpu`) |
