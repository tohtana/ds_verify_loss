...
# DeepSpeed from the third_party/DeepSpeed submodule (origin = public deepspeedai/DeepSpeed,
# pinned to a master commit so teammates can clone it; has DeepCompile), installed EDITABLE
# so edits under third_party/DeepSpeed are picked up without reinstalling. JIT-compiles ops
...
# accelerator), so run this on a GPU node. To send a DeepSpeed PR, work here and push to
# your fork (a 'fork' remote -> pengdurice/DeepSpeed is preconfigured in the submodule):
#   cd third_party/DeepSpeed && git checkout -b my-fix && ... && git push fork my-fix
